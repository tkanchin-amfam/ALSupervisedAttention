...
class RALSASampling:
	...
	def make_query(self, n=1):
		...
		if self.query == 'uncertain':
			...
			## entropy
			pred = np.sum(-pred * np.log(pred), axis=1)
			...
			## entropy task
			task_entropy = np.sum(-pred[0]*np.log(pred[0]), axis=1)

			## token attention task
			att_entropy = np.sum(-(pred[1]*np.log(pred[1]) + (1-pred[1])*np.log(1-pred[1])), axis=1)
